...
//------------------------------------------------------------------------------
void lib_weather::setRssUrl(const char *location, const char *url)
{
    if (_p_location)
        delete[] _p_location;
    if (_p_rss_url)
    ...
    _p_location = new char[strlen(location)+1];
    memset (_p_location,        0, strlen(location)+1);
    strncpy(_p_location, location, strlen(location));

    _p_rss_url = new char[strlen(url)+1];
    memset (_p_rss_url,   0, strlen(location)+1);
...
//------------------------------------------------------------------------------
lib_weather::lib_weather(WiFiClient *client, const char *location, const char *url)
{
    setClient(client);	setRssUrl(location, url);
}
